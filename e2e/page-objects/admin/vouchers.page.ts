...
import { clickRowAction, searchDataTable, submitFormAndWaitForSuccess, waitForModalClose, waitForModalOpen } from '../../helpers'
import { expectToastMessage } from '../../helpers/toast.helper'
import { BasePage } from '../base.page'

export class AdminVouchersPage extends BasePage {
  get heading() { return this.page.getByRole('heading', { name: adminSelectors.vouchers.heading }) }
  get createButton() { return this.page.getByRole('button', { name: adminSelectors.vouchers.addButton }) }
  get searchInput() { return this.page.getByRole('textbox', { name: adminSelectors.dataTable.searchInput }) }
  get dataTable() { return this.page.getByTestId('vouchers-table').or(this.page.getByRole('table').first()) }
  get codeInput() { return this.page.getByLabel(adminSelectors.vouchers.formDialog.code) }
  get discountSelection() { return this.page.getByRole('combobox', { name: adminSelectors.vouchers.formDialog.discountType }) }

  getVoucherCell(voucherCode: string | RegExp) { return this.page.getByRole('cell', { name: voucherCode }) }
  ...
  async searchVoucher(query: string) { await searchDataTable(this.page, query) }

  async fillVoucherForm(data: { code: string, discount: number, type: string, maxUses?: number }) {
  ...
  async submitVoucher() {
    await submitFormAndWaitForSuccess(this.page, adminSelectors.buttons.createVoucher, adminSelectors.notifications.voucherCreated)
    await waitForModalClose(this.page)
  }

  async createVoucher(data: { code: string, discount: number, type: string, maxUses?: number }) {
