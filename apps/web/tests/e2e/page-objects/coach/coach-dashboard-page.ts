...
export class CoachDashboardPage extends BasePage {
  ...
  get workshopsLink() {
    return this.page.getByRole('link', { name: dashboardContent.coach.navigation.workshops.text, exact: true })
  }

  get workshopsHeading() {
    return this.page.getByRole('heading', { name: dashboardContent.coach.navigation.workshops.text, exact: true })
  }

  get bookingsLink() {
    return this.page.getByRole('link', { name: dashboardContent.coach.navigation.bookings.text, exact: true })
  }

  get bookingsHeading() {
    return this.page.getByRole('heading', { name: dashboardContent.coach.navigation.bookings.text, exact: true })
  }

  get accountLink() {
    return this.page.getByRole('link', { name: dashboardContent.coach.navigation.account.text })
  }

  get upcomingSessionsHeading() {
    return this.page.locator('h2').filter({ hasText: dashboardContent.coach.sections.upcomingSessions })
  }

  get recentBookingsHeading() {
    return this.page.locator('h2').filter({ hasText: dashboardContent.coach.sections.recentBookings })
  }
