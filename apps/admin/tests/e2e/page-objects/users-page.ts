 ...
 * Users Page Object
 *
 * Handles user page interactions.
 * For assertions, use helpers from `helpers/users-assertions.ts`
 */
export class UsersPage extends BasePage {
  ...
  // Locators
  get heading() {
    return this.page.getByRole('heading', { name: adminSelectors.users.heading })
  }

  get createButton() {
    return this.page.getByRole('button', { name: adminSelectors.users.createButton })
  }
  ...
  get displayNameInput() {
    return this.page.getByLabel(adminSelectors.forms.displayName)
  }

  get firstNameInput() {
    return this.page.getByLabel(adminSelectors.forms.firstName)
  }

  get lastNameInput() {
    return this.page.getByLabel(adminSelectors.forms.lastName)
  }

  get emailInput() {
    return this.page.getByLabel(adminSelectors.forms.email)
  }

  get usernameInput() {
    return this.page.getByLabel(adminSelectors.forms.username)
  }

  get phoneNumberInput() {
    return this.page.getByLabel(adminSelectors.forms.phoneNumber)
  }

  get submitButton() {
    return this.page.getByRole('button', { name: adminSelectors.buttons.submit })
  }

  get continueButton() {
    return this.page.getByRole('button', { name: adminSelectors.buttons.continue })
  }

  get generatePasswordButton() {
    return this.page.getByRole('button', { name: adminSelectors.buttons.generatePassword })
  }
  ...
  getCreateUserDialog() {
    return this.page.getByRole('heading', { name: /Create|Add.*User/i })
  }
  ...
  // Actions
  async goto() {
    await this.navigate(adminRoutes.users)
  }

  async gotoUserDetail(userId: string) {
    await this.navigate(adminRoutes.userDetail(userId))
  }
  ...
  async submitUserForm() {
    await submitFormAndWaitForSuccess(
      this.page,
      adminSelectors.buttons.continue,
      adminSelectors.notifications.userCreated,
    )
  ...
  async clickDetailsTab() {
    await clickTab(this.page, adminSelectors.users.tabs.details)
  }

  async clickCoachProfileTab() {
    await clickTab(this.page, adminSelectors.users.tabs.coachProfile)
  }

  async clickWorkshopsTab() {
    await clickTab(this.page, adminSelectors.users.tabs.workshops)
  }

  async clickPayoutsTab() {
    await clickTab(this.page, adminSelectors.users.tabs.payouts)
  }

  async clickEditTab() {
    await clickTab(this.page, adminSelectors.users.tabs.edit)
  }

  /**
   * Get text from first user cell in table
   */
  async getFirstUserCellText(columnIndex: number = UsersPage.USER_EMAIL_COLUMN_INDEX): Promise<string | null> {
